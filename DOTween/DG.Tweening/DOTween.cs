...
namespace DG.Tweening
{
	public class DOTween
	{
		...
		private static IDOTweenInit Init(DOTweenSettings settings, bool? recycleAllByDefault, bool? useSafeMode, LogBehaviour? logBehaviour)
		{
			...
			{
				Debugger.Log("DOTween initialization (useSafeMode: " + DOTween.useSafeMode + ", recycling: " + (defaultRecyclable ? "ON" : "OFF") + ", logBehaviour: " + DOTween.logBehaviour + ")");
			}
			return instance;
		}
		...
		{
			return ApplyTo<Color, Color, ColorOptions>(getter, setter, new Color(0f, 0f, 0f, endValue), duration).SetOptions(alphaOnly: true);
		}
		...
		{
			return Shake(getter, setter, duration, new Vector3(strength, strength, strength), vibrato, randomness, ignoreZAxis, vectorBased: false);
		}
		...
		{
			return Shake(getter, setter, duration, strength, vibrato, randomness, ignoreZAxis: false, vectorBased: true);
		}
			...
			for (int k = 0; k < num2; k++)
			{
				if (k < num2 - 1)
				{
					...
					else
					{
						...
						{
							Quaternion rotation2 = Quaternion.AngleAxis(Random.Range(0f - randomness, randomness), Vector3.up);
		...
		{
			return TweenManager.FilteredOperation(OperationType.Complete, FilterType.All, null, optionalBool: false, 0f);
		}
		...
		public static int Complete(object targetOrId)
		{
			...
			return TweenManager.FilteredOperation(OperationType.Complete, FilterType.TargetOrId, targetOrId, optionalBool: false, 0f);
		}
		...
		public static int Kill(object targetOrId, bool complete = false)
		{
			...
			return num + TweenManager.FilteredOperation(OperationType.Despawn, FilterType.TargetOrId, targetOrId, optionalBool: false, 0f);
		}
		...
		{
			return TweenManager.FilteredOperation(OperationType.IsTweening, FilterType.TargetOrId, targetOrId, optionalBool: false, 0f) > 0;
		}
		...
		{
			return TweenManager.GetActiveTweens(playing: true);
		}

		public static List<Tween> PausedTweens()
		{
			return TweenManager.GetActiveTweens(playing: false);
		}
