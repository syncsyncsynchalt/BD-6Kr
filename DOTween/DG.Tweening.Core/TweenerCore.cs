	...
	public class TweenerCore<T1, T2, TPlugOptions> : Tweener where TPlugOptions : struct
	{
		...
		public override Tweener ChangeStartValue(object newStartValue, float newDuration = -1f)
		{
			...
			if (type != typeofT2)
			{
				if (Debugger.logPriority >= 1)
				{
					Debugger.LogWarning("ChangeStartValue: incorrect newStartValue type (is " + type + ", should be " + typeofT2 + ")");
				}
				return this;
			}
		...
		public override Tweener ChangeEndValue(object newEndValue, float newDuration = -1f, bool snapStartValue = false)
		{
			...
			if (type != typeofT2)
			{
				if (Debugger.logPriority >= 1)
				{
					Debugger.LogWarning("ChangeEndValue: incorrect newEndValue type (is " + type + ", should be " + typeofT2 + ")");
				}
				return this;
			}
		...
		public override Tweener ChangeValues(object newStartValue, object newEndValue, float newDuration = -1f)
		{
			...
			if (type != typeofT2)
			{
				if (Debugger.logPriority >= 1)
				{
					Debugger.LogWarning("ChangeValues: incorrect value type (is " + type + ", should be " + typeofT2 + ")");
				}
				return this;
			}
			if (type2 != typeofT2)
			{
				if (Debugger.logPriority >= 1)
				{
					Debugger.LogWarning("ChangeValues: incorrect value type (is " + type2 + ", should be " + typeofT2 + ")");
				}
				return this;
			}
		...
		internal sealed override void Reset()
		{
			...
			plugOptions = default(TPlugOptions);
			getter = null;
			setter = null;
