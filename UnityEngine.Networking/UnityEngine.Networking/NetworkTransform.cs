...
namespace UnityEngine.Networking
{
	...
	public class NetworkTransform : NetworkBehaviour
	{
		...
		{
			float num = (m_RigidBody3D != null) ? (m_RigidBody3D.position - m_PrevPosition).magnitude : ((!(m_RigidBody2D != null)) ? (base.transform.position - m_PrevPosition).magnitude : (m_RigidBody2D.position - (Vector2)m_PrevPosition).magnitude);
			if (num > 1E-05f)
			{
				return true;
			}
