namespace UnityEngine.UI
{
	...
	public class GridLayoutGroup : LayoutGroup
	{
		...
		public override void CalculateLayoutInputHorizontal()
		{
			...
			{
				num2 = (num = m_ConstraintCount);
			}
			else if (m_Constraint == Constraint.FixedRowCount)
			{
				num2 = (num = Mathf.CeilToInt((float)base.rectChildren.Count / (float)m_ConstraintCount - 0.001f));
			}
			else
			{
				num2 = 1;
				num = Mathf.CeilToInt(Mathf.Sqrt(base.rectChildren.Count));
			}
			float num3 = base.padding.horizontal;
			Vector2 cellSize = this.cellSize;
			float x = cellSize.x;
			Vector2 spacing = this.spacing;
			float num4 = num3 + (x + spacing.x) * (float)num2;
			...
			float num6 = num5 + (x2 + spacing3.x) * (float)num;
