...
namespace UnityEngine.UI
{
	...
	public class Dropdown : Selectable, IEventSystemHandler, IPointerClickHandler, ISubmitHandler, ICancelHandler
	{
		...
		public void Show()
		{
			...
			for (int j = 0; j < 4; j++)
			{
				Vector3 point = rectTransform4.InverseTransformPoint(array[j]);
				if (!rectTransform4.rect.Contains(point))
				{
					flag = true;
					break;
				}
			...
			{
				RectTransform rectTransform5 = m_Items[k].rectTransform;
				...
				float num2 = vector.y + size.y * (float)(m_Items.Count - 1 - k);
				float y = size.y;
