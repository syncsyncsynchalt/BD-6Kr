	...
	public class Image : MaskableGraphic, ICanvasRaycastFilter, ISerializationCallbackReceiver, ILayoutElement
	{
		...
		public virtual float preferredWidth
		{
			get
			{
				...
				{
					Vector2 minSize = DataUtility.GetMinSize(overrideSprite);
		...
		private void GenerateSlicedSprite(VertexHelper toFill)
		{
			...
			if (overrideSprite != null)
			{
				...
				a = DataUtility.GetPadding(overrideSprite);
				a2 = overrideSprite.border;
			}
			else
			{
				vector = Vector4.zero;
			...
			Rect pixelAdjustedRect = GetPixelAdjustedRect();
			a2 = GetAdjustedBorders(a2 / pixelsPerUnit, pixelAdjustedRect);
			a /= pixelsPerUnit;
			s_VertScratch[0] = new Vector2(a.x, a.y);
			s_VertScratch[3] = new Vector2(pixelAdjustedRect.width - a.z, pixelAdjustedRect.height - a.w);
			s_VertScratch[1].x = a2.x;
			s_VertScratch[1].y = a2.y;
			s_VertScratch[2].x = pixelAdjustedRect.width - a2.z;
			s_VertScratch[2].y = pixelAdjustedRect.height - a2.w;
			for (int i = 0; i < 4; i++)
			{
			...
			for (int i = 0; i < 2; i++)
			{
				if (!(local[i] <= adjustedBorders[i]))
				{
					...
					if (rect.size[i] - local[i] <= adjustedBorders[i + 2])
					{
						int index2 = index = i;
						num = local[index];
						local[index2] = num - (rect.size[i] - rect2.size[i]);
						continue;
					}
					if (type == Type.Sliced)
					{
