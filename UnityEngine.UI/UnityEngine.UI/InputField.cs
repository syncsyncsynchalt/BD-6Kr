	...
	public class InputField : Selectable, IEventSystemHandler, IPointerClickHandler, IBeginDragHandler, IDragHandler, IEndDragHandler, IUpdateSelectedHandler, ISubmitHandler, ICanvasElement
	{
		...
		private int GetUnclampedCharacterLineFromPosition(Vector2 pos, TextGenerator generator)
		{
			...
			for (int i = 0; i < generator.lineCount; i++)
			{
				...
				float num2 = (float)uILineInfo.height / m_TextComponent.pixelsPerUnit;
		...
		public override void OnPointerDown(PointerEventData eventData)
		{
			...
			if (allowInput)
			{
				...
				int num3 = caretSelectPositionInternal = (caretPositionInternal = GetCharacterIndexFromPosition(pos) + m_DrawStart);
			}
			UpdateLabel();
			eventData.Use();
		}
		...
		private void SetDrawRangeToContainCaretPosition(int caretPos)
		{
			...
			if (multiLine)
			{
				...
				if (m_DrawEnd <= caretPos)
				{
					...
					{
						int num4 = num2;
						UILineInfo uILineInfo = lines[num3];
						num2 = num4 - uILineInfo.height;
						if (num2 < 0)
						{
							break;
						}
				...
				int num6 = num5;
				m_DrawEnd = GetLineEndPosition(cachedInputTextGenerator, num6);
				int num7 = num2;
				UILineInfo uILineInfo2 = lines[num6];
				num2 = num7 - uILineInfo2.height;
				while (true)
				{
					if (num6 < lines.Count - 1)
					{
						num6++;
						int num8 = num2;
						UILineInfo uILineInfo3 = lines[num6];
						if (num8 < uILineInfo3.height)
						{
							break;
						}
						m_DrawEnd = GetLineEndPosition(cachedInputTextGenerator, num6);
						int num9 = num2;
						UILineInfo uILineInfo4 = lines[num6];
						num2 = num9 - uILineInfo4.height;
						continue;
					}
					if (num5 > 0)
					{
						...
						if (num10 < uILineInfo5.height)
						{
							break;
						}
						m_DrawStart = GetLineStartPosition(cachedInputTextGenerator, num5);
						int num11 = num2;
						UILineInfo uILineInfo6 = lines[num5];
						num2 = num11 - uILineInfo6.height;
						continue;
					}
					break;
				}
			...
			if (caretPos > m_DrawEnd || (caretPos == m_DrawEnd && m_DrawStart > 0))
			{
				...
				for (m_DrawStart = m_DrawEnd - 1; m_DrawStart >= 0; m_DrawStart--)
				{
					...
					if (num13 + uICharInfo.charWidth > size.x)
					{
						break;
					}
					float num14 = num12;
					UICharInfo uICharInfo2 = characters[m_DrawStart];
					num12 = num14 + uICharInfo2.charWidth;
				}
				m_DrawStart++;
			}
			else
			...
			while (m_DrawEnd < cachedInputTextGenerator.characterCountVisible)
			{
				...
				num12 = num15 + uICharInfo3.charWidth;
				if (num12 > size.x)
				{
					break;
				}
		...
		private void OnFillVBO(Mesh vbo)
		{
			using (VertexHelper vertexHelper = new VertexHelper())
			{
				...
				else
				{
					...
					Vector2 a = m_TextComponent.PixelAdjustPoint(zero);
		...
		private float SumLineHeights(int endLine, TextGenerator generator)
		{
			...
			for (int i = 0; i < endLine; i++)
			{
				float num2 = num;
				UILineInfo uILineInfo = generator.lines[i];
				num = num2 + (float)uILineInfo.height;
			}
			return num;
		}
		...
		private void GenerateHightlight(VertexHelper vbo, Vector2 roundingOffset)
		{
			...
			{
				UILineInfo uILineInfo = cachedInputTextGenerator.lines[0];
				num5 = uILineInfo.height;
			}
			...
			for (int i = num; i <= num2 && i < cachedTextGenerator.characterCountVisible; i++)
			{
				if (i + 1 == lineEndPosition || i == num2)
				{
					...
					vector = new Vector2(uICharInfo.cursorPos.x / m_TextComponent.pixelsPerUnit, m_TextComponent.rectTransform.rect.yMax - num6 / m_TextComponent.pixelsPerUnit);
