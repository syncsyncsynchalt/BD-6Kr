...
public class DepthOfField34 : PostEffectsBase
{
	...
	public void OnRenderImage(RenderTexture source, RenderTexture destination)
	{
		...
		if (simpleTweakMode)
		{
			...
			{
				num4 = FocalDistance01(focalPoint);
			}
			...
			if (num5 != 0)
			{
				num5 = ((focalPoint > GetComponent<Camera>().nearClipPlane + Mathf.Epsilon) ? 1 : 0);
			}
			flag = ((byte)num5 != 0);
		}
		else
		{
			if ((bool)objectFocus)
			{
				Vector3 vector2 = GetComponent<Camera>().WorldToViewportPoint(objectFocus.position);
				vector2.z /= GetComponent<Camera>().farClipPlane;
			...
			if (num6 != 0)
			{
				num6 = ((focalPoint > GetComponent<Camera>().nearClipPlane + Mathf.Epsilon) ? 1 : 0);
			}
			flag = ((byte)num6 != 0);
		}
