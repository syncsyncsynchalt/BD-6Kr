...
public class CameraMotionBlur : PostEffectsBase
{
	...
	public void OnRenderImage(RenderTexture source, RenderTexture destination)
	{
		...
		float maxVelocity2 = maxVelocity;
		bool flag = false;
		...
		if (filterType == MotionBlurFilter.Reconstruction || flag || filterType == MotionBlurFilter.ReconstructionDisc)
		{
			...
			height = divRoundUp(temporary.height, (int)maxVelocity);
			value = temporary.width / num;
		}
		else
		{
			num = divRoundUp(temporary.width, (int)maxVelocity);
		...
		motionBlurMaterial.SetFloat("_MaxVelocity", value);
		motionBlurMaterial.SetFloat("_MaxRadiusOrKInPaper", value);
		...
		if (filterType == MotionBlurFilter.CameraMotion)
		{
			...
			float num3 = Vector3.Angle(transform.up, prevFrameUp) / GetComponent<Camera>().fieldOfView * ((float)source.width * 0.75f);
			zero.x = rotationScale * num3;
			num3 = Vector3.Angle(transform.forward, prevFrameForward) / GetComponent<Camera>().fieldOfView * ((float)source.width * 0.75f);
			zero.y = rotationScale * num2 * num3;
			num3 = Vector3.Angle(transform.forward, prevFrameForward) / GetComponent<Camera>().fieldOfView * ((float)source.width * 0.75f);
			zero.z = rotationScale * (1f - num2) * num3;
		...
		else if (filterType == MotionBlurFilter.ReconstructionDX11 && !flag)
		{
			...
			dx11MotionBlurMaterial.SetFloat("_MaxRadiusOrKInPaper", value);
