	...
	public class TextEditor
	{
		...
		public bool DeleteLineBack()
		{
			...
			if (cursorIndex != num)
			{
				...
				int num6 = selectIndex = (cursorIndex = num);
				return true;
			}
			return false;
		}
		...
		public void MoveUp()
		{
			...
			int num2 = cursorIndex = (selectIndex = style.GetCursorStringIndex(position, content, graphicalCursorPos));
			if (cursorIndex <= 0)
			{
				ClearCursorPos();
			}
		...
		public void MoveLineStart()
		{
			...
			while (num2-- != 0)
			{
				if (content.text[num2] == '\n')
				{
					num6 = (selectIndex = (cursorIndex = num2 + 1));
					return;
				}
			}
			num6 = (selectIndex = (cursorIndex = 0));
		}
		...
		{
			int num2 = selectIndex = (cursorIndex = content.text.Length);
		}
		...
		public void MoveParagraphBackward()
		{
			...
			if (cursorIndex > 1)
			{
				int num3 = selectIndex = (cursorIndex = content.text.LastIndexOf('\n', cursorIndex - 2) + 1);
			}
			else
			{
				int num3 = selectIndex = (cursorIndex = 0);
		...
		{
			int cursorIndex = this.cursorIndex;
			graphicalCursorPos = style.GetCursorPixelPosition(new Rect(0f, 0f, position.width, position.height), content, cursorIndex);
			Rect rect = style.padding.Remove(position);
			Vector2 vector = style.CalcSize(content);
			Vector2 vector2 = new Vector2(vector.x, style.CalcHeight(content, position.width));
		...
		public void Copy()
		{
			...
			{
				string text2 = GUIUtility.systemCopyBuffer = ((cursorIndex >= selectIndex) ? content.text.Substring(selectIndex, cursorIndex - selectIndex) : content.text.Substring(cursorIndex, selectIndex - cursorIndex));
			}
		}
