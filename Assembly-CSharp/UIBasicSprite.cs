...
public abstract class UIBasicSprite : UIWidget
{
	...
	private void TiledFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		{
			float num2 = drawingDimensions.x;
			float num3 = num + a.y;
			float y2 = vector.w;
			...
			for (; num2 < drawingDimensions.z; num2 += a.x)
			{
				float num4 = num2 + a.x;
				float x2 = vector.z;
				if (num4 > drawingDimensions.z)
				{
					x2 = Mathf.Lerp(vector.x, vector.z, (drawingDimensions.z - num2) / a.x);
					num4 = drawingDimensions.z;
				}
				verts.Add(new Vector3(num2, num));
				verts.Add(new Vector3(num2, num3));
