...
public static class NGUIText
{
	...
	public static void Print(string text, BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		if (bitmapFont != null)
		{
			...
			num5 = rect.width / (float)bitmapFont.texWidth;
			num6 = rect.height / (float)bitmapFont.texHeight;
		}
		for (int i = 0; i < length; i++)
		{
			int num8 = text[i];
			...
			if (bMSymbol != null)
			{
				num11 = num + (float)bMSymbol.offsetX * fontScale;
				num12 = num11 + (float)bMSymbol.width * fontScale;
				num13 = 0f - (num2 + (float)bMSymbol.offsetY * fontScale);
				num14 = num13 - (float)bMSymbol.height * fontScale;
				if (Mathf.RoundToInt(num + (float)bMSymbol.advance * fontScale) > regionWidth)
				{
					if (num == 0f)
					{
						return;
			...
			GlyphInfo glyphInfo = GetGlyph(num8, prev);
			if (glyphInfo == null)
			{
				continue;
			}
			...
			num11 = glyphInfo.v0.x + num;
			num14 = glyphInfo.v0.y - num2;
			num12 = glyphInfo.v1.x + num;
			num13 = glyphInfo.v1.y - num2;
			float num15 = glyphInfo.advance;
			if (finalSpacingX < 0f)
			{
				num15 += finalSpacingX;
			}
			if (Mathf.RoundToInt(num + num15) > regionWidth)
			{
				if (num == 0f)
				{
					return;
			...
			num += ((sub != 0) ? ((finalSpacingX + glyphInfo.advance) * 0.75f) : (finalSpacingX + glyphInfo.advance));
			if (IsSpace(num8))
			{
				continue;
			}
			...
			if (cols != null)
			{
				if (glyphInfo.channel == 0 || glyphInfo.channel == 15)
				{
					if (gradient)
					{
						float num17 = num7 + glyphInfo.v0.y / fontScale;
						float num18 = num7 + glyphInfo.v1.y / fontScale;
						num17 /= num7;
						num18 /= num7;
			...
			if (flag && strike)
			{
				num14 = (0f - num2 + glyphInfo2.v0.y) * 0.75f;
				num13 = (0f - num2 + glyphInfo2.v1.y) * 0.75f;
			}
			else
			{
				num14 = 0f - num2 + glyphInfo2.v0.y;
				num13 = 0f - num2 + glyphInfo2.v1.y;
			}
			if (bold)
			{
				for (int num31 = 0; num31 < 4; num31++)
				{
					...
					verts.Add(new Vector3(num + num32, num13 + num33));
					verts.Add(new Vector3(num + num32, num14 + num33));
				}
			}
			else
			{
			...
			if (gradient)
			{
				float num34 = num7 + glyphInfo2.v0.y / fontScale;
				float num35 = num7 + glyphInfo2.v1.y / fontScale;
				num34 /= num7;
				num35 /= num7;
