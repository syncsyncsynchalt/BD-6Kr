...
public class UICamera : MonoBehaviour
{
	...
	private static Rigidbody FindRootRigidbody(Transform trans)
	{
		while (trans != null)
		{
			if (((Component)trans).GetComponent<UIPanel>() != null)
			{
				return null;
			}
	...
	public static bool Raycast(Vector3 inPos)
	{
		for (int i = 0; i < list.size; i++)
		{
			...
			if (uICamera.eventType == EventType.World_3D)
			{
                ...
                // if (!Physics.Raycast(ray, ref lastHit, enter, num))
				// {
				//	continue;
				// }
				...
				{
					Rigidbody val = FindRootRigidbody(hoveredObject.transform);
					if ((UnityEngine.Object)val != null)
					{
						hoveredObject = ((Component)val).gameObject;
					}
				}
				return true;
			}
			...
			{
				RaycastHit[] array = Physics.RaycastAll(ray, enter, num);
				if (array.Length > 1)
				{
			...
			else
			{
				if (uICamera.eventType == EventType.World_2D)
				{
					...
					Collider2D collider2D = Physics2D.OverlapPoint((Vector2)point, num);
					if (!collider2D)
					{
						continue;
					}
					...
					{
						Rigidbody2D val2 = FindRootRigidbody2D(hoveredObject.transform);
						if ((UnityEngine.Object)val2 != null)
						{
							hoveredObject = ((Component)val2).gameObject;
						}
					}
					return true;
				}
				...
				Collider2D[] array2 = Physics2D.OverlapPointAll((Vector2)lastWorldPosition, num);
				if (array2.Length > 1)
				{
	...
	private void Awake()
	{
		...
		if (Application.platform == RuntimePlatform.Android || Application.platform == RuntimePlatform.PSP2 || Application.platform == RuntimePlatform.IPhonePlayer || Application.platform == RuntimePlatform.WP8Player || Application.platform == RuntimePlatform.BlackBerryPlayer)
		{
			useTouch = true;
			useMouse = false;
			useKeyboard = false;
	...
	public void ProcessTouch(bool pressed, bool released)
	{
		...
		if (currentTouch.pressed != null)
		{
			...
			if (longPressTooltip && currentTouch.pressed == currentTouch.current && currentTouch.clickNotification != 0 && !currentTouch.dragStarted && currentTouch.deltaTime > tooltipDelay)
			{
				currentTouch.clickNotification = ClickNotification.None;
